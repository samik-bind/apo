...
class TestCacheWriteIsPricedForEveryProvider:
    ...
    def test_cache_write_uses_provider_rate(self, session: Session) -> None:
        ...
        GPT-5.6 carried a 1.25x premium here until it was re-measured on
        2026-08-10: a cold call reporting 15017 ``cache_write_tokens`` billed
        ``upstream_inference_prompt_cost`` $0.01877425, i.e. $1.2502/1M — the
        uncached input rate, not 1.25x it.
        """
        load_default_prices(session)
