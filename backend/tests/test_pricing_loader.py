...
class TestCacheWriteIsPricedForEveryProvider:
    ...
    def test_cache_write_priced_wherever_cache_read_is(self, session: Session) -> None:
        ...
        assert not missing, (
            "these entries price cache reads but not cache writes, so "
            "cache-creation tokens silently cost $0: " + ", ".join(missing)
        )

    def test_non_anthropic_cache_write_equals_input_rate(self, session: Session) -> None:
        """These providers charge no write premium, but the tokens that populate
        a cache are billed as ordinary input on the call that writes them."""
        load_default_prices(session)
        for name in ("gpt-5.6-luna", "gpt-5.6-terra", "gemini-3.6-flash", "glm-5.2"):
