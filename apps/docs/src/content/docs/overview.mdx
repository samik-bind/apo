...
The loop can be driven by a developer, CI, or a coding agent. apo owns one Task Run and its evidence. The caller owns the code change and decides whether to run again.

That separation is why apo is a testing framework rather than an eval dashboard: it returns a control signal you can act on, not just a score to monitor.
