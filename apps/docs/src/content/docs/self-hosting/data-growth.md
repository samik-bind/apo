...
There is no undo. Export what you care about first — `apo runs show <run-id> --json` dumps a run with its checks, and the dashboard's trace bulk-export ships full traces — or bookmark the trace.
:::
